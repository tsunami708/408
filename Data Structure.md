...
# 图
- 考察图基本特性
	 *无向图n个顶点，若边大于n-1,比存在环  ; n个顶点非连通考虑n-1个顶点构成完全图 ; n个顶点的有向图强连通至少n条边*
	- 定义
		[6.1.2 : 2, 3, 4, 10, 13, 14, 15]
	- 顶点和边关系
		1. 非连通图至少几边/几点
		2. 连通图至少几边
		3. 环图至少几边
		4. 入度出度
		[6.1.2 : 1, 5, 6, 7, 8, 9, 16, 17, 18]
	- 连通分量(极**大**连通子图) —— 生成树(极**小**连通子图)
		1. 有向图中只有入边或出边的点单独为1个强连通分量
		[6.1.2 : 11, 12] 
		[6.3.4 : 13]
- 图的存储方式
		[6.2.6 : 1, 7]
	...
	- 邻接表
		[6.2.6 : 9~16]
	- 十字链表(有向图)
		tailvex-headvex-hlink-tlink(-info) **{thht}**
		data-firstin-firstout  **{dio}**
		[6.2.6 : 18]
	- 邻接多重表(无向图)
		ivex-ilink-jvex-jlink(-info) **{iijj}**
		data-firstedge
		[6.2.6 : 17, 20]
- 遍历图
		*考察不同物理结构下的时空复杂度*
	 无向图连通分量个数等于BFS/DFS次数-->判断连通性
	- 广度优先遍历
		**BFS可以确定非带权图单元最短路径**
		[6.3.4 : 1, 5, 15, 16]
	- 深度优先遍历
		**DFS可以确定图是否存在环;BFS只能确定无向图是否存在环**
		[6.3.4 : 3, 7, 8, 11, 12, 17, 18]
	[6.3.4 : 2, 4, 6, 9, 10, 14]
- DAG表达式
- 最小生成树
	1. **当不存在权值相同的边时,最小生成树唯一**
	- Prim - 找点
	- Kruskal - 找边
- 最短路径 (*不允许出现负权回路,但Floyd允许负权值*)
	- Dijkstra 
	- Floyd
- 拓扑排序
	**可确定有向图是否有回路**
